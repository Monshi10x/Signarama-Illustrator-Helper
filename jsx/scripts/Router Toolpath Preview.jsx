...
The script creates an offset tool-centre path, a translucent cutter-width
sweep, an exact copy of the desired edge, and optional cutter-position circles
at corners a round bit cannot reproduce.
...
(function () {
    ...
    function buildPreview(documentRef, sourceData, settings, outputLayer) {
        ...
        for (sourceIndex = 0;
            ...
            if (record.capped || record.vertices.length < 3) {
                stats.invalidPaths += 1;
                continue;
            }
            var offset = machiningOffset(
                record.vertices,
                record.depth,
                settings.cutMode,
                radius
            ...
            var route = offsetClosedPolyline(record.vertices, offset);
            if (route.invalid || route.commands.length < 2) {
                stats.invalidPaths += 1;
                continue;
            }
            ...
            var centrelinePath = null;

            if (settings.showSweep) {
                sweepPath = createPathFromCommands(
                    pathGroup,
                    route.commands,
                    true
                );
                if (sweepPath) {
                    styleSweepPath(sweepPath, settings.diameterPoints);
                }
            }
            if (settings.showDesired) {
                desiredPath = createExactDesiredPath(pathGroup, record.snapshot);
                styleDesiredPath(desiredPath);
            }
            if (settings.showCentreline) {
                centrelinePath = createPathFromCommands(
                    pathGroup,
                    route.commands,
                    true
                );
            ...
            try {
                if (sweepPath) {
                    sweepPath.zOrder(ZOrderMethod.SENDTOBACK);
                }
                if (desiredPath) {
                    desiredPath.zOrder(ZOrderMethod.BRINGTOFRONT);
                }
                if (centrelinePath) {
                    centrelinePath.zOrder(ZOrderMethod.BRINGTOFRONT);
                }
            } catch (ignoreOrder) {
            }
    ...
    function showDialog(documentRef, sourceData) {
        ...
        toleranceRow.add("statictext", undefined, "Curve tolerance:");
        ...
        var information = dialog.add("statictext", undefined,
            "Grey circles mark normal cutter-limited corners. Orange circles " +
            "mark tight corners whose cutter contact extends beyond a nearby edge.",
            { multiline: true });
        ...
        okButton.onClick = function () {
            ...
            try {
                ...
                if (!settings.showDesired && !settings.showSweep &&
