...
(function () {
    ...
    var NATIVE_EXPAND_BATCH_SIZE = 300;
    ...
    function batchCount(itemCount, batchSize) {
        if (itemCount <= 0) {
            return 0;
        }
    ...
    function expandNativeAppearanceBatch(documentRef, items) {
        var startIndex = 0;
        ...
        while (startIndex < items.length) {
            ...
            var endIndex = Math.min(
                items.length,
                startIndex + NATIVE_EXPAND_BATCH_SIZE
            );
            var selectedCount = 0;
            var itemIndex;
            for (itemIndex = startIndex;
                    itemIndex < endIndex;
                    itemIndex += 1) {
                try {
    ...
    function offsetTargetInsideHolder(holder) {
        var directChildren = [];
        var childIndex;
        for (childIndex = 0;
                childIndex < holder.pageItems.length;
                childIndex += 1) {
            try {
                if (holder.pageItems[childIndex].parent === holder) {
                    directChildren.push(holder.pageItems[childIndex]);
                }
            } catch (ignoreParent) {
            }
        }
        ...
        if (directChildren.length > 1) {
            var targetGroup = holder.groupItems.add();
            targetGroup.name = "Second offset input";
            for (childIndex = directChildren.length - 1;
                    childIndex >= 0;
                    childIndex -= 1) {
                directChildren[childIndex].move(
                    targetGroup,
        ...
        throw new Error("The first Offset Path pass produced no artwork.");
    }
    ...
    function getFlattenedRecords(sourceData, tolerancePoints) {
        var cacheKey = String(tolerancePoints);
        if (sourceData.flattenCache &&
                sourceData.flattenCache.key === cacheKey) {
            return sourceData.flattenCache.records;
        }
        ...
        assignCompoundDepths(records);
        sourceData.flattenCache = {
            key: cacheKey,
            records: records
        };
        return records;
    ...
    function buildPreview(documentRef, sourceData, settings, outputLayer) {
        ...
        var flattenedRecords = getFlattenedRecords(
            sourceData,
        ...
        for (sourceIndex = 0;
                ...
                sourceIndex += 1) {
            if (flattenedRecords[sourceIndex].capped) {
                stats.cappedPaths += 1;
            }
        }
        ...
        var jobs = [];
        var firstPassExpansionItems = [];

        function rememberNativeError(error) {
            if (!stats.nativeErrorMessage) {
                stats.nativeErrorMessage = error && error.message ?
                    error.message : String(error);
            }
        }

        try {
            /* Stage 1: create every first-pass live offset. Direct applyEffect
               calls are cheap; no expandStyle menu command runs in this loop. */
            for (sourceIndex = 0;
                    sourceIndex < flattenedRecords.length;
                    sourceIndex += 1) {
                ...
                if (settings.showCutterCircles && !record.capped) {
                ...
                try {
                    job.centreHolder = pathGroup.groupItems.add();
                    job.centreHolder.name = "Native centreline base";
                    var centreSeed = createExactDesiredPath(
                        job.centreHolder,
                        record.snapshot
                    );
                    if (settings.showDesired) {
                        job.desiredPath = duplicateArtwork(
                            centreSeed,
                            pathGroup
                        );
                        styleDesiredPath(job.desiredPath);
                    }

                    if (settings.cutMode !== 2) {
                        job.sequence = nativeOffsetSequence(
                            record.depth,
                            settings.cutMode,
                            radius
                        );
                        var needsCentreOutput = settings.showSweep ||
                            settings.showCentreline;
                        var needsResultOutput = settings.showResult;
                        if (needsCentreOutput || needsResultOutput) {
                            applyNativeOffsetEffect(
                                centreSeed,
                                job.sequence.first
                            );
                        }
                        if (needsResultOutput) {
                            job.resultHolder = pathGroup.groupItems.add();
                            job.resultHolder.name = "Native two-pass result";
                            var resultFirstSeed = duplicateArtwork(
                                centreSeed,
                                job.resultHolder
                            );
                            firstPassExpansionItems.push(resultFirstSeed);
                        }
                        if (needsCentreOutput) {
                            firstPassExpansionItems.push(centreSeed);
                        } else {
                            job.centreHolder.remove();
                            job.centreHolder = null;
                        }
                    } else if (!settings.showSweep &&
                            !settings.showCentreline) {
                        job.centreHolder.remove();
                        job.centreHolder = null;
                    }
                    ...
                    job.failed = true;
                    rememberNativeError(stageOneError);
                }
                jobs.push(job);
            }

            /* One batch expansion replaces two expandStyle calls per path. */
            if (firstPassExpansionItems.length > 0) {
                stats.nativeExpansionCalls += expandNativeAppearanceBatch(
                    documentRef,
                    firstPassExpansionItems
                );
            }

            /* Stage 2: apply the reverse offset to every expanded first pass,
               then expand the entire selection in one more batch. */
            var secondPassExpansionItems = [];
            var jobIndex;
            for (jobIndex = 0; jobIndex < jobs.length; jobIndex += 1) {
                var secondJob = jobs[jobIndex];
                if (secondJob.failed || !secondJob.resultHolder) {
                    continue;
                }
                try {
                    var secondTarget = offsetTargetInsideHolder(
                        secondJob.resultHolder
                    );
                    applyNativeOffsetEffect(
                        secondTarget,
                        secondJob.sequence.second
                    );
                    secondPassExpansionItems.push(secondTarget);
                } catch (stageTwoError) {
                    secondJob.failed = true;
                    rememberNativeError(stageTwoError);
                }
            }
            if (secondPassExpansionItems.length > 0) {
                stats.nativeExpansionCalls += expandNativeAppearanceBatch(
                    documentRef,
                    secondPassExpansionItems
                );
            }

            /* Stage 3: style and arrange the already-expanded batch output. */
            for (jobIndex = 0; jobIndex < jobs.length; jobIndex += 1) {
                var jobToFinish = jobs[jobIndex];
                if (jobToFinish.failed) {
                    try {
                        jobToFinish.pathGroup.remove();
                ...
                var resultArtwork = jobToFinish.resultHolder;
                if (settings.showSweep && jobToFinish.centreHolder) {
                    sweepArtwork = duplicateArtwork(
                        jobToFinish.centreHolder,
                        jobToFinish.pathGroup
                    );
                    styleArtwork(
                        sweepArtwork,
                        "Cutter sweep",
                ...
                if (settings.showCentreline && jobToFinish.centreHolder) {
                ...
                if (jobToFinish.centreHolder) {
                    jobToFinish.centreHolder.remove();
                    jobToFinish.centreHolder = null;
                }
                ...
                if (settings.showCutterCircles) {
                    ...
                    for (circleIndex = 0;
                            circleIndex <
                                jobToFinish.route.limitedCenters.length;
                            circleIndex += 1) {
                        createCutterCircle(
                            jobToFinish.pathGroup,
                            jobToFinish.route.limitedCenters[circleIndex].point,
                            radius,
                            jobToFinish.route.limitedCenters[circleIndex].tight
                        );
                    }
                }
                ...
                try {
                    ...
                    if (jobToFinish.desiredPath) {
                        jobToFinish.desiredPath.zOrder(
                ...
                stats.limitedCorners +=
                    jobToFinish.route.limitedCenters.length;
                stats.tightCorners += jobToFinish.route.tightCount;
            }
        } catch (fatalBatchError) {
            documentRef.selection = null;
            try {
                root.remove();
            } catch (ignoreRootRemove) {
            }
    ...
    function settingsSignature(settings) {
        return [
            ...
            settings.tolerancePoints,
            settings.cutMode,
            settings.showDesired ? 1 : 0,
            settings.showResult ? 1 : 0,
            settings.showSweep ? 1 : 0,
            settings.showCentreline ? 1 : 0,
            settings.showCutterCircles ? 1 : 0
        ].join("|");
    }
    ...
    function showDialog(documentRef, sourceData) {
        ...
        function updateStatus(stats) {
            ...
            if (stats.nativeExpansionCalls > 0) {
                message += "; " + stats.nativeExpansionCalls +
                    " batched expansion call(s)";
            }
        ...
        function refreshPreview(showErrors) {
            ...
            try {
                ...
                previewSignature = settingsSignature(settings);
        ...
        cutModeDropdown.onChange = function () {
            showResult.enabled = cutModeDropdown.selection.index !== 2;
            refreshPreview(false);
        };
        ...
        okButton.onClick = function () {
            ...
            try {
                ...
                var hasEffectiveResult = settings.showResult &&
                    settings.cutMode !== 2;
                if (!settings.showDesired && !hasEffectiveResult &&
                        !settings.showSweep &&
                        !settings.showCentreline &&
                        !settings.showCutterCircles) {
                    throw new Error("Select at least one display option.");
                }
                var requestedSignature = settingsSignature(settings);
                if (!previewRoot || !latestStats ||
