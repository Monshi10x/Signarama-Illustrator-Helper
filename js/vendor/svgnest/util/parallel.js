(function (root) {
	...
	function withSynchronousEnv(rootObj, ns, env, fn) {
		var target = rootObj || {};
		var hadGlobal = Object.prototype.hasOwnProperty.call(target, 'global');
		var previousGlobal = target.global;
		target.global = target.global || {};
		var hadNs = Object.prototype.hasOwnProperty.call(target.global, ns);
		var previousNs = target.global[ns];
		target.global[ns] = env || {};
		try {
			return fn();
		} finally {
			if (hadNs) {
				target.global[ns] = previousNs;
			} else {
				delete target.global[ns];
			}
			if (hadGlobal) {
				target.global = previousGlobal;
			} else {
				try {
					delete target.global;
				} catch (_eDeleteGlobal) {
	...
	Parallel.prototype._spawnWorker = function (cb, env) {
		...
		if (!isNode && root && root.__srhSvgNestDisableWorkers) {
			return undefined;
		}
		if (isNode) {
		...
		this.operation.then(function () {
			...
			if (wrk !== undefined) {
				...
				setImmediate(function () {
					try {
						that.data = withSynchronousEnv(root, that.options.envNamespace, env, function () {
							return cb(that.data);
						});
						newOp.resolve(null, that.data);
					} catch (e) {
	...
	Parallel.prototype._spawnMapWorker = function (i, cb, done, env, wrk) {
		...
		if (wrk !== undefined) {
			...
			setImmediate(function () {
				try {
					that.data[i] = withSynchronousEnv(root, that.options.envNamespace, env, function () {
						return cb(that.data[i]);
					});
					done();
				} catch (e) {
					done(e);
	...
	Parallel.prototype._spawnReduceWorker = function (data, cb, done, env, wrk) {
		...
		if (wrk !== undefined) {
			...
			setImmediate(function () {
				try {
					that.data[that.data.length] = withSynchronousEnv(root, that.options.envNamespace, env, function () {
						return cb(data);
					});
					done();
				} catch (e) {
