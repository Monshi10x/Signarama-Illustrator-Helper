...
(function(root){
	...
	function SvgNest(){
		...
		function srhLog(message){
			try{
				if(typeof window !== 'undefined' && window.__srhNestLogHook){
					window.__srhNestLogHook(String(message || ''));
				}
			}
			catch(_eSvhLog){}
		}

		function srhMark(eventName, extra){
			try{
				...
				var current = null;
				try{
					current = JSON.parse(localStorage.getItem('srhNestRuntimeState') || 'null');
				}
				catch(_eSvhMarkRead){
					current = null;
				}
				var now = Date.now();
				var isNestEvent = /^(nest-|result-|worker-)/.test(String(eventName || ''));
				var payload = extra || {};
				payload.event = String(eventName || '');
				payload.at = now;
				payload.lastNestEvent = isNestEvent ? String(eventName || '') : (current && current.lastNestEvent ? current.lastNestEvent : '');
				payload.lastNestAt = isNestEvent ? now : (current && current.lastNestAt ? current.lastNestAt : 0);
				localStorage.setItem('srhNestRuntimeState', JSON.stringify(payload));
			}
			catch(_eSvhMark){}
		}

		function srhMaxWorkers(){
			try{
				if(typeof window !== 'undefined' && window.__srhSvgNestMaxWorkers){
					return Math.max(1, parseInt(window.__srhSvgNestMaxWorkers, 10) || 1);
				}
			}
			catch(_eSvhWorkers){}
			return 1;
		...
		this.start = function(progressCallback, displayCallback){						
			...
			try{
				...
				for(var ps=0; ps<parts.length; ps++){
					...
					var pid = '';
					try{ pid = pe.getAttribute ? (pe.getAttribute('id') || '') : ''; } catch(_ePsId){ pid = ''; }
					var pchildren = 0;
					try{ pchildren = pe.childNodes ? pe.childNodes.length : 0; } catch(_ePsCh){ pchildren = 0; }
					partSummary.push('#' + ps + ':' + pe.tagName + (pid ? ('#' + pid) : '') + '[children=' + pchildren + ']');
				}
				if(partSummary.length){
					srhLog('SVGnest: top-level part candidates => ' + partSummary.join(', '));
				}
			}
			catch(_ePartSummary){}
			...
			srhLog('SVGnest: parsed ' + parts.length + ' SVG child node(s) into ' + (tree ? tree.length : 0) + ' nestable polygon(s).');
			srhMark('worker-start-parsed', {partCount: tree ? tree.length : 0});
		...
		this.launchWorkers = function(tree, binPolygon, config, progressCallback, displayCallback){
			...
			var p = new Parallel(nfpPairs, {
				...
				maxWorkers: srhMaxWorkers(),
				evalPath: 'js/vendor/svgnest/util/eval.js'
			});
			...
			p.map(function(pair){
				...
				srhMark('worker-placement-map-start', {population: placelist ? placelist.length : 0});
				srhLog('SVGnest: placement map started.');
				p2.map(worker.placePaths).then(function(placements){
					srhMark('worker-placement-map-success', {resultCount: placements && placements.length ? placements.length : 0});
					srhLog('SVGnest: placement map success.');
					...
					if(!best || bestresult.fitness < best.fitness){
						...
						if(typeof window !== 'undefined' && window.__srhSvgNestUsePlacementOnlyCallback){
							...
							srhLog('SVGnest: applyPlacement begin.');
					...
					srhMark('worker-placement-map-error', {message: err && err.message ? err.message : String(err || '')});
					if(typeof console !== 'undefined' && console.error){
						console.error('SVGnest placement worker failed', err);
					}
				});
			}, function (err) {
				srhMark('worker-nfp-map-error', {message: err && err.message ? err.message : String(err || '')});
				if(typeof console !== 'undefined' && console.error){
					console.error('SVGnest NFP worker failed', err);
				}
			});
		}
		...
		this.getParts = function(paths){
			...
			for(i=0; i<numChildren; i++){
				...
				var sourcePartId = null;
				try{
					if(paths[i] && paths[i].getAttribute){
						sourcePartId = paths[i].getAttribute('data-srh-part-id') || paths[i].getAttribute('id') || paths[i].getAttribute('class') || null;
					}
				}
				catch(_ePolySourceId){
					sourcePartId = null;
				}
				...
				if(poly && poly.length > 2 && Math.abs(GeometryUtil.polygonArea(poly)) > config.curveTolerance*config.curveTolerance){
					poly.source = i;
					poly.sourcePartId = sourcePartId;
					polygons.push(poly);
					try{
						srhLog('SVGnest: polygon[' + i + '] accepted | points=' + poly.length + ' | area=' + Math.round(Math.abs(GeometryUtil.polygonArea(poly))) + ' | sourcePart=' + (sourcePartId || 'n/a') );
					}
					catch(_ePolyAccept){}
				}
				else{
					try{
						var polyPoints = poly && poly.length ? poly.length : 0;
						var polyArea = poly && poly.length ? Math.abs(GeometryUtil.polygonArea(poly)) : 0;
						srhLog('SVGnest: polygon[' + i + '] rejected | points=' + polyPoints + ' | area=' + Math.round(polyArea) + ' | sourcePart=' + (sourcePartId || 'n/a'));
					}
					catch(_ePolyReject){}
				}
			}
			...
			function toTree(list, idstart){
				...
				for(i=0; i<list.length; i++){
					...
					for(j=0; j<list.length; j++){
						...
						if(list[j].sourcePartId && p.sourcePartId && list[j].sourcePartId !== p.sourcePartId){
							continue;
						}
